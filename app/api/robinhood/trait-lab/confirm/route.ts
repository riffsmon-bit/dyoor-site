...
export async function POST(request: Request) {
  try {
    ...
    assertHoodYoorRateLimit(`confirm:${wallet || "invalid"}:${clientIp(request)}`, 6, 60_000);
    return json(200, await confirmHoodYoorRerollPreview(body));
  } catch (error) {
    return json(Number((error as { status?: number })?.status || 500), {
      ok: false,
      error: hoodYoorPublicErrorMessage(error, "HoodYØØR gasless acceptance failed."),
    });
  }
}
