      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 pb-24 pt-10 sm:px-6 sm:pt-16">
        <div className="grid items-end gap-8 lg:grid-cols-[1.25fr_0.75fr]">
          <div>
            ...
            <h1 className="mt-5 max-w-3xl text-5xl font-black uppercase leading-[0.86] tracking-[-0.07em] sm:text-7xl lg:text-8xl">
              Droid<br /><span className="text-[#c7ff00]">Squad</span>
            </h1>
        ...
        {config?.configured && !config.activationEnabled ? (
          <div className="mt-6 rounded-2xl border border-amber-300/25 bg-amber-300/[0.07] p-4">
            <p className="text-xs font-black uppercase tracking-[0.15em] text-amber-200">Activation staged</p>
            <p className="mt-2 text-sm font-semibold leading-6 text-white/50">The deterministic account infrastructure is configured, but this chain&apos;s holder activation gate remains off pending its approved release or canary. Portfolio reads remain non-custodial and no reward, strategy, agent, or bridge feature is implied.</p>
          </div>
        ) : null}
  ...
  return (
    <Link href={`${basePath}/${droid.tokenId}`} className="group overflow-hidden rounded-[1.5rem] border border-white/10 bg-[#0c110b] transition hover:-translate-y-1 hover:border-[#c7ff00]/35 hover:shadow-[0_25px_75px_rgba(0,0,0,.45),0_0_35px_rgba(199,255,0,.06)]">
      <div className="relative aspect-square overflow-hidden bg-[#11180d]">
        ...
        <span className={`absolute right-3 top-3 rounded-full border px-3 py-2 text-[0.55rem] font-black uppercase tracking-[0.12em] backdrop-blur-md ${droid.active ? "border-[#c7ff00]/35 bg-black/55 text-[#c7ff00]" : "border-white/15 bg-black/55 text-white/45"}`}>{droid.active ? "Online" : "Offline"}</span>
      </div>
      <div className="p-5">
        <div className="flex items-start justify-between gap-4"><div><p className="text-[0.55rem] font-black uppercase tracking-[0.17em] text-[#c7ff00]/55">{droid.directive}</p><h3 className="mt-1 text-2xl font-black uppercase tracking-[-0.045em]">Droid #{droid.tokenId}</h3></div><span className="text-xl text-[#c7ff00] transition group-hover:translate-x-1" aria-hidden="true">→</span></div>
