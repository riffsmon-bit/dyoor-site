...
contract UnlistedPullProbe {
    function pull(LiveParent parent, address from) external {
        parent.safeTransferFrom(from, address(this), 11);
    }
}
...
contract MonadWrapperForkTest {
    ...
    bytes32 private constant S2_HASH =
    ...
    function testRealValidatorRejectsApprovedUnlistedPullOperator() public {
        ...
        vm.prank(owner);
        parent.approve(address(probe), 11);
        vm.expectRevert(bytes4(0x1de5204e));
        probe.pull(parent, owner);
    ...
    function testRealS2WrapReceiptMissionRoundTripUnwrapAndV1Isolation() public {
        ...
        bytes32 v1Code = V1.codehash;
        require(LegacyAccount(V1).owner() == owner);
        vm.chainId(31337);
        MissionMintLab minter = new MissionMintLab();
        DroidControlReceiptLab wrapper =
            new DroidControlReceiptLab(IWrappedParentLab(S2), minter, Adapter.disabledVenue());
        bytes memory intent = abi.encode(wrapper.WRAP_INTENT());
        vm.prank(owner);
        parent.safeTransferFrom(owner, address(wrapper), 11, intent);
        ...
        DroidMissionAccountCoreLab.Limits memory l = DroidMissionAccountCoreLab.Limits(
            ...
            2,
            0,
            keccak256("LOCAL fork free mint")
        );
        vm.prank(owner);
        ...
        vm.prank(owner);
        wrapper.transferFrom(owner, B, 11);
        vm.prank(B);
        wrapper.transferFrom(B, owner, 11);
        vm.prank(RUNNER);
        vm.expectRevert();
        ...
        vm.chainId(143);
        require(
            LegacyAccount(V1).owner() == owner && V1.balance == v1Balance && V1.codehash == v1Code
        );
