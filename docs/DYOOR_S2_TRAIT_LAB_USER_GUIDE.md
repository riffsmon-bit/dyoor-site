...
- Harvested Energy is Energy emitted by harvest events.
- Energy Balance is the spendable Energy balance Trait Lab can debit.
...
If a wallet has harvested Energy but cannot reroll, that harvested amount still needs to be represented in the spendable Energy balance. The reconciliation/admin credit flow repairs old harvests that were indexed but not credited to the spendable balance.
