...
- ASK popup clock-skew fix: a live unsigned challenge on September 6 returned HTTP 200, a numeric block/expiry and valid nonce, but its expiry was 126,723ms ahead of this Mac's clock. The old browser limit of exactly 120,000ms rejected it before calling the wallet. The response now includes server `issuedAt`; the client checks the exact two-minute lifetime and independently reconstructs the signed message, using conservative monotonic elapsed time from before the request instead of comparing device and server wall clocks. The server's absolute two-minute expiry, one-use proof, origin/request binding and canonical ownership checks are unchanged. Old preview tabs must reload. Progress text distinguishes owner checking, awaiting a wallet signature, and server verification.
- Six client-boundary regressions cover seven-second/large clock skew, malformed fields and extended lifetimes, origin/operation mismatch, elapsed timeout before signing/submission, authoritative server expiry even if the browser timer stalls, and wallet changes. Signatures in these tests use freshly generated test-only keys, never the user's wallet. All 30 ASK/provider tests pass.
- Clock-skew patch verification: TypeScript, lint and optimized webpack build passed (only the existing optional Privy dependency warnings). Roster/UI 19, ASSIST 52 and wallet/mobile 7 tests passed. Rendered UI checks passed at 1440/390/360px; the mobile screenshot was reviewed. A browser run of the actual request client with a seven-second-skew fixture reached exactly one stub signing call and all three progress stages. This is not evidence of a real MetaMask signature or live AI reply.
...
- The next upload was still over that limit. After checking exact equal values, precedence in runtime code and absence of an inherited all-context value, removed only the Deploy Preview values for `DYOOR_S2_METADATA_GATEWAY` (shadowed by `IPFS_GATEWAY_URL`) and `NEXT_PUBLIC_DYOOR_TRAIT_LAB_ENABLE_LEADERBOARD` (shadowed by `DYOOR_TRAIT_LAB_ENABLE_LEADERBOARD`). Their primary variables remain; all other deployment-context values were verified unchanged. Restore either preview alias by copying its primary variable's preview value. Added a behavior regression test for unchanged metadata source and enabled leaderboard; 24 ASK/provider tests pass. This is configuration deduplication, not a CID/baseURI/metadata migration. A second local optimized build passed with AI non-secret selection enabled and no credential available locally.
