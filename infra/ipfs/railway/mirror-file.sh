...
if [ "$root" = bafybeifz4gwsvqbypeki3wwwmmvng2z2lusqjndevfqemzibisk266vepq ] && [ -f "$index" ]; then
  expected=$(jq -r --arg name "$file" '.Links[] | select(.Name == $name) | .Hash["/"]' "$index")
  if [ -n "$expected" ] && curl -fsSL --connect-timeout 15 --max-time 60 \
    ...
    actual=$(ipfs add -Q --cid-version=1 --pin=false "$temporary")
    if [ "$actual" = "$expected" ] && curl -fsS --max-time 30 "http://127.0.0.1:8081/ipfs/$root/$file" -o /dev/null; then
      touch "$marker"
      exit 0
    fi
  fi
