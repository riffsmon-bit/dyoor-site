...
export const LEGACY_DYOOR_S2_TESTNET_CONTRACT = "0xcE586aA467F6351bf819DbF134BC69947125CD92";
...
function configuredS2ChainId() {
  const testnetEnabled = /^(1|true|yes|on)$/i.test(String(
    process.env.DYOOR_S2_ENABLE_TESTNET
    || process.env.NEXT_PUBLIC_DYOOR_S2_ENABLE_TESTNET
    || "",
  ));
  const parsed = Number(
...
function s2ContractAddress() {
  ...
  if (!testnetEnabled && (!configured || configured === legacyTestnet)) {
    return mainnet;
  }

  return configured || mainnet;
}
