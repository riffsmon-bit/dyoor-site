...
export async function readAssistState(rpc) {
  if (BigInt(await rpc("eth_chainId", [])) !== 143n) throw Error("Switch your wallet to Monad mainnet (143).");
  ...
  return { owner, active, minted: active ? await read(m.badge, "hasMinted", [m.account]) : false,
...
export async function reconcileAssistReceipt(rpc, pending) {
  if (!["ACTIVATE", "MINT"].includes(pending?.plan?.kind)) throw Error("Unsupported canary action.");
