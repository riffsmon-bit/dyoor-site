...
export const validArtworkTokenId = tokenId => typeof tokenId === "string" && /^[1-9][0-9]{0,3}$/.test(tokenId) && Number(tokenId) <= 3333;

export function parseLiveArtwork(metadata, tokenId) {
  if (!validArtworkTokenId(tokenId) || !metadata || typeof metadata !== "object" ||
...
export async function readLiveArtwork(tokenId, fetcher = fetch) {
  if (!validArtworkTokenId(tokenId)) throw new Error("Unsupported Season 2 token");
