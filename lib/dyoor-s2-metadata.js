...
const LOCKED_TRAIT_TYPES = new Set(["Background", "Droid"]);
const DISABLED_OVERRIDE_TRAIT_TYPES = new Set(["Special"]);
const GUARANTEED_TRAIT_TYPES = new Set(["Eyes", "Mouth"]);
const EMPTY_TRAIT_VALUES = new Set(["", "none", "null", "undefined", "unknown", "n a", "na"]);
const CANONICAL_TRAIT_TYPES = new Map([
  ["background", "Background"],
  ["droid", "Droid"],
  ["conditions", "Conditions"],
  ["sticker", "Stickers/Body art"],
  ["stickers", "Stickers/Body art"],
  ["stickers body art", "Stickers/Body art"],
  ["body art", "Stickers/Body art"],
  ["clothes", "Clothes"],
  ["mouth", "Mouth"],
  ["eyes", "Eyes"],
  ["hat", "Hat"],
...
const LEGACY_SPECIAL_REPAIR_TOKEN_IDS = new Set(["486"]);
...
function configuredS2ChainId(env = process.env) {
  ...
  const chainId = Math.floor(parsed);
  if (env.NODE_ENV === "production" && chainId !== DEFAULT_S2_CHAIN_ID) {
    throw new Error(`Production metadata configuration must use Monad mainnet chain ${DEFAULT_S2_CHAIN_ID}.`);
  }
  return chainId;
}
...
function configuredS2ContractAddress(env = process.env) {
  ...
  if (!configured) return DEFAULT_DYOOR_S2_MAINNET_CONTRACT;
  if (env.NODE_ENV === "production" && configured !== DEFAULT_DYOOR_S2_MAINNET_CONTRACT) {
    const detail = configured === LEGACY_DYOOR_S2_TESTNET_CONTRACT ? "legacy Monad testnet" : "non-mainnet";
    throw new Error(`Production metadata configuration cannot use the ${detail} contract.`);
  }
  return configured;
}
...
function resolveRuntimeTraitOverride(overrides, tokenId, env = process.env) {
  ...
  if (!legacy || typeof legacy !== "object" || !legacyOverrideMatchesDeployment(legacy, tokenId, env)) return null;
  return legacy;
}

function legacyOverrideMatchesDeployment(override, tokenId, env = process.env) {
  if (!override || typeof override !== "object" || Array.isArray(override)) return false;
  if (override.tokenId != null && String(override.tokenId) !== String(tokenId)) return false;
  if (override.chainId != null && String(override.chainId) !== String(configuredS2ChainId(env))) return false;
  if (override.contractAddress != null
    && normalizeAddressKey(override.contractAddress) !== configuredS2ContractAddress(env)) return false;
  // Pre-scope records have no deployment fields. They are accepted only in the
  // trusted runtime override store shape produced by the historical Trait Lab.
  return Boolean(
    override.updatedAt
    && override.updatedBy
    && override.attributes
...
function remoteMetadataBaseUrls(env = process.env) {
  const configured = [
    env.DYOOR_S2_METADATA_BASE_URL,
    env.DYOOR_S2_METADATA_URL,
    env.NEXT_PUBLIC_DYOOR_S2_METADATA_BASE_URL,
  ...
  const preferredGateway = String(env.IPFS_GATEWAY_URL || env.NEXT_PUBLIC_IPFS_GATEWAY_URL || "").trim().replace(/\/+$/, "");
  const bases = [];
  if (preferredGateway && cid) bases.push(`${preferredGateway}/ipfs/${cid}`);

  const gateway = String(env.DYOOR_S2_METADATA_GATEWAY || env.NEXT_PUBLIC_DYOOR_S2_METADATA_GATEWAY || "").trim().replace(/\/+$/, "");
  if (gateway && cid) bases.push(`${gateway}/ipfs/${cid}`.replace(/\/+$/, ""));

  if (cid) bases.push(normalizeBaseUrl(DEFAULT_S2_METADATA_BASE_URL));
  return unique(bases);
}
...
function normalizeAttribute(attribute) {
  ...
  const rawTraitType = String(attribute.trait_type || "").trim();
  const traitType = CANONICAL_TRAIT_TYPES.get(normalizeComparableTraitValue(rawTraitType)) || rawTraitType;
...
function normalizeComparableTraitValue(value) {
  return String(value ?? "")
    .trim()
    .replace(/[^a-z0-9]+/gi, " ")
    .replace(/\s+/g, " ")
...
function isEmptyOverrideTraitValue(value) {
  return EMPTY_TRAIT_VALUES.has(normalizeComparableTraitValue(value));
}

function hasDisabledSpecialOverride(attributes) {
  return Boolean(attributes
    && typeof attributes === "object"
    && !Array.isArray(attributes)
    && !isEmptyOverrideTraitValue(attributes.Special));
}

function shouldStripSavedOverrideAttribute(traitType, value) {
  if (LOCKED_TRAIT_TYPES.has(traitType)) return true;
  if (DISABLED_OVERRIDE_TRAIT_TYPES.has(traitType) && !isEmptyOverrideTraitValue(value)) return true;
  if (GUARANTEED_TRAIT_TYPES.has(traitType) && isEmptyOverrideTraitValue(value)) return true;
  return false;
}

function shouldIgnoreStoredOverrideAttribute(traitType, value) {
  if (LOCKED_TRAIT_TYPES.has(traitType)) return true;
  if (GUARANTEED_TRAIT_TYPES.has(traitType) && isEmptyOverrideTraitValue(value)) return true;
  return false;
}

function shouldIgnoreStoredTraitOverride(overrides, tokenId) {
  return LEGACY_SPECIAL_REPAIR_TOKEN_IDS.has(String(tokenId))
    && hasDisabledSpecialOverride(overrides?.attributes);
}

export function sanitizeOverrideAttributes(attributes) {
  if (!attributes || typeof attributes !== "object" || Array.isArray(attributes)) return attributes;
  return Object.fromEntries(Object.entries(attributes).filter(([traitType, value]) => (
    !shouldStripSavedOverrideAttribute(traitType, value)
  )));
}

function sanitizeTraitOverride(override) {
  if (!override || typeof override !== "object" || Array.isArray(override)) return override;
  const attributes = sanitizeOverrideAttributes(override.attributes);
  if (attributes === override.attributes) return override;
...
function applyMergedTraitCompatibility(_attributes, overrideAttributes, _metadata) {
  if (!overrideAttributes || typeof overrideAttributes !== "object" || Array.isArray(overrideAttributes)) return;

  // Compatibility is validated before overrides are written. Merging metadata should
  // not erase independent wearable slots such as Hat when Accessories changes.
}
...
export async function getRemoteBaseMetadata(tokenId, env = process.env) {
  for (const baseUrl of remoteMetadataBaseUrls(env)) {
...
export async function saveRuntimeTraitOverride(tokenId, override) {
  ...
  const sanitizedOverride = sanitizeTraitOverride(override);
  const next = {
    ...overrides,
    [key]: {
      ...sanitizedOverride,
      ...
      updatedAt: sanitizedOverride?.updatedAt || new Date().toISOString(),
    },
  };
...
export function mergeMetadata(baseMetadata, overrides, tokenId, config = getMetadataConfig()) {
  const activeOverrides = shouldIgnoreStoredTraitOverride(overrides, tokenId) ? undefined : overrides;
  const metadata = cloneJson(baseMetadata || {});
  const version = activeOverrides?.version || 1;

  metadata.name = activeOverrides?.name || metadata.name || `${config.collectionName} #${tokenId}`;
  metadata.description = activeOverrides?.description || metadata.description || config.description;
  metadata.image = activeOverrides?.image || normalizeTokenImageUri(metadata.image, tokenId, config);
  if (activeOverrides?.image) {
    metadata.properties = metadata.properties && typeof metadata.properties === "object" ? metadata.properties : {};
    metadata.properties.files = [{ uri: activeOverrides.image, type: "image/png" }];
    metadata.properties.category = metadata.properties.category || "image";
  }
  ...
  if (activeOverrides?.attributes && typeof activeOverrides.attributes === "object") {
    for (const [traitType, value] of Object.entries(activeOverrides.attributes)) {
      if (shouldIgnoreStoredOverrideAttribute(traitType, value)) continue;
      setAttribute(attributes, traitType, value);
    }
    applyMergedTraitCompatibility(attributes, activeOverrides.attributes, metadata);
  }
...
export function metadataIsAuthoritative(metadata, baseFound = false) {
  if (!baseFound || !metadata || typeof metadata !== "object" || Array.isArray(metadata)) return false;
  const values = new Map(normalizeAttributes(metadata.attributes).map((attribute) => [
    attribute.trait_type,
    normalizeComparableTraitValue(attribute.value),
  ]));
  return ["Background", "Droid", "Eyes", "Mouth"].every((traitType) => {
    const value = values.get(traitType);
    return Boolean(value && !EMPTY_TRAIT_VALUES.has(value));
  });
}

export const METADATA_UNAVAILABLE_ERROR = "Current metadata unavailable. Reroll disabled.";
...
export function buildTokenMetadata(tokenId, config = getMetadataConfig()) {
  ...
  return {
    ...
    authoritative: metadataIsAuthoritative(metadata, base.found),
  };
}
