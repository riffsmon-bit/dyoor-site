...
function applyMergedTraitCompatibility(attributes, overrideAttributes) {
  ...
  if (overrideTraits.has("Hat") && !isEmptyOverrideTraitValue(hat)) {
    const clearsAccessories = isBandannaTraitValue(accessories);
    const clearsAccessories2 = isBandannaTraitValue(accessories2);
...
export function mergeMetadata(baseMetadata, overrides, tokenId, config = getMetadataConfig()) {
  ...
  if (activeOverrides?.attributes && typeof activeOverrides.attributes === "object") {
    ...
    applyMergedTraitCompatibility(attributes, activeOverrides.attributes);
  }
