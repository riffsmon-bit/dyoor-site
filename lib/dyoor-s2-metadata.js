...
export const DEFAULT_S2_METADATA_CID = "bafybeictzn54rbfnqd7vdqfhtu2vwy2bp7vnqj4zp2tswsrosge43frapi";
...
export async function buildTokenMetadataAsync(tokenId, config) {
  ...
  const localBase = uploadedBase ? null : getBaseMetadata(tokenId, runtimeConfig);
  const remoteBase = uploadedBase || localBase?.found ? null : await getRemoteBaseMetadata(tokenId);
  const base = uploadedBase || (localBase?.found ? localBase : null) || remoteBase || localBase || getBaseMetadata(tokenId, runtimeConfig);
