...
const FACE_COVER_FALLBACK_MOUTH = "Displeased";
...
function isBandannaTraitValue(value) {
...
function getAttributeValue(attributes, traitType) {
  const attribute = attributes.find((entry) => entry.trait_type === traitType);
  return attribute?.value;
}

function initialDnaTraitMap(metadata) {
...
function applyMergedTraitCompatibility(attributes, overrideAttributes, metadata) {
  if (!overrideAttributes || typeof overrideAttributes !== "object" || Array.isArray(overrideAttributes)) return;

  const overrideTraits = new Set(Object.keys(overrideAttributes));
  const initialTraits = initialDnaTraitMap(metadata);
  const hat = getAttributeValue(attributes, "Hat");
  const accessories = getAttributeValue(attributes, "Accessories");
  const accessories2 = getAttributeValue(attributes, "Accessories 2");

  if (overrideTraits.has("Hat") && !isEmptyOverrideTraitValue(hat)) {
    const clearsAccessories = isBandannaTraitValue(accessories) || isBandannaTraitValue(initialTraits.Accessories);
    const clearsAccessories2 = isBandannaTraitValue(accessories2) || isBandannaTraitValue(initialTraits["Accessories 2"]);
    if (clearsAccessories) setAttribute(attributes, "Accessories", "None");
    if (clearsAccessories2) setAttribute(attributes, "Accessories 2", "None");
    if ((clearsAccessories || clearsAccessories2) && isEmptyOverrideTraitValue(getAttributeValue(attributes, "Mouth"))) {
      setAttribute(attributes, "Mouth", FACE_COVER_FALLBACK_MOUTH);
    }
  }

  if ((overrideTraits.has("Accessories") && isBandannaTraitValue(accessories))
    || (overrideTraits.has("Accessories 2") && isBandannaTraitValue(accessories2))) {
    if (!isEmptyOverrideTraitValue(hat)) setAttribute(attributes, "Hat", "None");
  }
}
