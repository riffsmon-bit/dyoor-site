...
exports.handler = async () => {
  const enabled = /^(1|true|yes|on)$/i.test(
    String(process.env.DYOOR_WORLD_PUSH_ENABLED || ""),
  );
  ...
  if (!enabled || secret.length < 32) {
    return {
      ...
      body: JSON.stringify({
        ...
        skipped: true,
        reason: enabled
          ? "The dYOOR World automation key is not configured."
          : "DYOOR_WORLD_PUSH_ENABLED is off.",
      }),
    };
  }

  const response = await fetch(`${siteOrigin()}/api/dyoor-world/automation/push`, {
    method: "POST",
    headers: {
