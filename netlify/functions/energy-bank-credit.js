...
export default async (request) => {
  try {
    ...
    const signerKey = normalizePrivateKey(process.env.ENERGY_CREDIT_SIGNER_PRIVATE_KEY || "");
    if (!signerKey) {
      return json(500, { ok: false, error: "Missing ENERGY_CREDIT_SIGNER_PRIVATE_KEY" });
    }
    ...
    if (alreadyUsed) return json(409, { ok: false, error: "This harvest is already credited on-chain." });

    const nonce = await bank.nonces(user);
    const deadline = BigInt(Math.floor(Date.now() / 1000) + 15 * 60);
    const wallet = new ethers.Wallet(signerKey);

    const domain = {
      name: "DYOOREnergyBank",
      version: "1",
      chainId: CHAIN_ID,
      verifyingContract: energyBankAddress
    };
    const types = {
      CreditAuthorization: [
