...
import {
  ...
  getRemoteBaseMetadata,
  getRuntimeTraitOverrides,
  metadataIsAuthoritative,
  mergeMetadata,
...
test("Hat overrides preserve Bandanna accessory layers", () => {
  const metadata = mergeMetadata({
    ...baseMetadata,
    attributes: baseMetadata.attributes.map((attribute) => (
  ...
  assert.equal(traits.Mouth, "None");
  assert.equal(traits["Accessories 2"], "Bandana Black");
  assert.equal(traits.Background, "Eye Sea U-Project M.A.D.");
  assert.equal(traits.Droid, "Lime Green");
  assert.equal(traits["Metadata Version"], "2");
});
...
test("dedicated S2 metadata source takes precedence over generic IPFS gateway", async () => {
  ...
  globalThis.fetch = async (url) => {
    ...
    return new Response(JSON.stringify({
      name: "D.Y.O.O.R #11",
      image: "ipfs://base/11.png",
      attributes: [
        { trait_type: "Background", value: "Eye Sea" },
        { trait_type: "Droid", value: "Lime Green" },
        { trait_type: "Eyes", value: "Okay" },
        { trait_type: "Mouth", value: "Joint Mouth" },
      ],
  ...
  try {
    const result = await getRemoteBaseMetadata(11, {
      DYOOR_S2_METADATA_BASE_URL: "https://dedicated.example/s2",
      IPFS_GATEWAY_URL: "https://generic.example",
    });
    assert.equal(result.source, "https://dedicated.example/s2/11");
    assert.equal(requests[0], "https://dedicated.example/s2/11");
  } finally {
...
test("historical unscoped override remains readable without replacing scoped precedence", async () => {
  const override = await getRuntimeTraitOverrides(1);
  assert.equal(override?.tokenId || "1", "1");
  assert.equal(runtimeTraitOverrideKey(11, {
    DYOOR_S2_CHAIN_ID: "143",
    DYOOR_S2_CONTRACT_ADDRESS: "0x349d8eb480c92cf75371fba5c6344a4d11b9103a",
  }), "143:0x349d8eb480c92cf75371fba5c6344a4d11b9103a:11");
});
